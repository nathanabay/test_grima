...
export function PageHeader({
  ...
  return (
    <div className="mb-4">
      {breadcrumb && <div className="mb-1 text-small text-ink-subtle">{breadcrumb}</div>}
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          <h1 className="text-title text-ink">{title}</h1>
          {subtitle && <p className="mt-1 max-w-3xl text-small text-ink-muted">{subtitle}</p>}
        </div>
...
export function Loading({ label = 'Loading' }: { label?: string }) {
  return (
    <div className="flex items-center gap-2 px-1 py-6 text-small text-ink-muted" role="status">
...
export function Stat({
  ...
  const body = (
    <>
      ...
      <div className={`mt-1 num text-2xl font-semibold ${valueTone}`}>{value}</div>
      {sub && <div className="mt-1 text-small text-ink-subtle">{sub}</div>}
    </>
  );
  ...
  if (onClick) {
    return (
      <button type="button" onClick={onClick} className={`${shell} w-full hover:shadow-panel`}>
        {body}
      </button>
    );
  }
...
export function Drawer({
  ...
  if (!open) return null;
  const w = width === 'xl' ? 'max-w-3xl' : width === 'lg' ? 'max-w-2xl' : 'max-w-lg';
    ...
    <div className="fixed inset-0 z-40 flex justify-end">
      ...
      >
        <header className="flex items-start justify-between gap-3 border-b border-border px-4 py-3">
          ...
          <button className="btn-quiet btn-sm" onClick={onClose} aria-label="Close panel">
            Close
          </button>
        </header>
...
export function Field({
  ...
  return (
    <label className="block">
      ...
      {children}
      {error ? (
        <span className="mt-1 block text-small text-danger" role="alert">{error}</span>
      ) : hint ? (
