...
export type StatusTone =
  | 'available' | 'low' | 'out' | 'near' | 'expired' | 'quarantine' | 'recall'
  | 'blocked' | 'cold' | 'controlled' | 'transit' | 'pending' | 'approved'
  | 'rejected' | 'info' | 'neutral';
...
const STATUS_TONE: Record<string, StatusTone> = {
  // Stock and batch
  AVAILABLE: 'available', RELEASED: 'available', IN_STOCK: 'available',
  LOW_STOCK: 'low', LOW: 'low',
  OUT_OF_STOCK: 'out', OUT: 'out',
  NEAR_EXPIRY: 'near', EXPIRING: 'near',
  EXPIRED: 'expired',
  QUARANTINED: 'quarantine', QUARANTINE: 'quarantine', UNDER_REVIEW: 'quarantine',
  RECALLED: 'recall', RECALL: 'recall',
  BLOCKED: 'blocked', DAMAGED: 'blocked', DESTROYED: 'blocked',
  RETURNED: 'blocked', DISPOSED: 'blocked', INACTIVE: 'blocked',
  // Movement
  IN_TRANSIT: 'transit', DISPATCHED: 'transit', SHIPPED: 'transit',
  PARTIALLY_RECEIVED: 'transit', PICKING: 'transit', PACKED: 'transit',
  // Decisions
  DRAFT: 'pending', SUBMITTED: 'pending', PENDING: 'pending',
  PROCUREMENT_REVIEW: 'pending', FINANCE_REVIEW: 'pending', NEW: 'pending',
  APPROVED: 'approved', COMPLETED: 'approved', RECEIVED: 'approved',
  CLOSED: 'approved', POSTED: 'approved', DISPENSED: 'approved',
  DELIVERED: 'approved', VERIFIED: 'approved', ACTIVE: 'approved',
  REJECTED: 'rejected', CANCELLED: 'blocked', FAILED: 'rejected',
  REVERSED: 'rejected', VOID: 'rejected',
  // Regulatory
  CONTROLLED: 'controlled', COLD_CHAIN: 'cold',
  // Severity, shared by incidents, recalls and excursions
  CRITICAL: 'out', HIGH: 'expired', MEDIUM: 'near', LOW_SEVERITY: 'info',
  CLASS_I: 'recall', CLASS_II: 'expired', CLASS_III: 'near',
};
...
export function SeverityBadge({ level }: { level: string }) {
  ...
  const tone: StatusTone =
    up === 'CRITICAL' ? 'out' : up === 'HIGH' ? 'expired' : up === 'MEDIUM' ? 'near' : 'info';
...
export function ExpiryBadge({ days }: { days: number | null | undefined }) {
  if (days === null || days === undefined) return <span className="text-ink-subtle">—</span>;
  if (days < 0) {
    return <StatusBadge tone="expired">Expired {Math.abs(days)}d ago</StatusBadge>;
  }
  const tone: StatusTone = days <= 30 ? 'near' : days <= 90 ? 'low' : 'available';
...
export function QuantityCell({ value, unit }: { value: unknown; unit?: string }) {
