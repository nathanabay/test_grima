...
function personaFor(user: AuthUser | null): Persona {
  ...
  if (roles.some((r) => ["PHARMACIST", "PHARMACY_TECHNICIAN"].includes(r)))
    return "pharmacist";
  if (roles.some((r) => ["WAREHOUSE_MANAGER", "STOREKEEPER"].includes(r)))
    return "warehouse";
...
const PERSONA_SUB: Record<Persona, string> = {
  pharmacist:
    "Dispensing, stock warnings and anything that would stop you handing medicine over safely.",
  warehouse:
    "The work waiting on the floor, and the bins and batches behind it.",
  procurement:
    "What needs ordering, what is late, and how suppliers are performing.",
  executive:
    "Value, margin and the exposures worth knowing about this morning.",
  general:
    "Every figure is computed live from the stock ledger. Click a card to act on it.",
};
    ...
    <>
        ...
        action={
          scope.branch && (
            <span className="text-small text-ink-muted">
              Showing{" "}
              <span className="font-medium text-ink">{scope.branch.name}</span>
            </span>
          )
        }
      />
      ...
      {data && (
        <div className="space-y-4">
          {persona === "pharmacist" && (
            <PharmacistPanels data={data} user={user} />
          )}
          {persona === "warehouse" && (
            <WarehousePanels data={data} scope={scope} />
          )}
...
function PharmacistPanels({
  ...
  const queue = useApi<any>(
    "/dispensing/prescriptions?status=VERIFIED&pageSize=8",
    [],
  );
  const c = data.cards;
    ...
    <>
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        <Stat
          ...
          value={qty(queue.data?.total ?? 0)}
          href="/dispensing"
          sub="Verified prescriptions in the queue"
          tone={(queue.data?.total ?? 0) > 0 ? "info" : "neutral"}
        />
        <Stat
          label="Expiring soon"
          value={qty(c.nearExpiry)}
          tone={c.nearExpiry > 0 ? "warn" : "neutral"}
          sub={`${money(c.expiryValueAtRisk)} at risk`}
          href="/inventory/expiry"
        />
        <Stat
          label="Quarantined"
          value={qty(c.quarantined)}
          tone={c.quarantined > 0 ? "warn" : "neutral"}
          sub="Held pending a QA decision"
          href="/batches"
        />
        <Stat
          label="Under recall"
          value={qty(c.recalled)}
          tone={c.recalled > 0 ? "danger" : "neutral"}
          sub={
            c.openRecalls ? `${c.openRecalls} open recall(s)` : "No open recall"
          }
          href="/recalls"
        />
      </div>
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        <Card
          ...
          description="Oldest first — a prescription waiting is a patient waiting."
        >
...
function WarehousePanels({
  ...
  const exceptions = useApi<any>(
    warehouseId
      ? `/warehouse/tasks/exceptions?warehouseId=${warehouseId}`
      : null,
    [warehouseId],
  );
  const occupancy = useApi<any>(
    warehouseId ? `/warehouse/occupancy?warehouseId=${warehouseId}` : null,
    [warehouseId],
  );
  ...
  return (
    <>
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        <Stat
          label="Open tasks"
          value={qty(tasks.data?.total ?? 0)}
          href="/warehouse"
          sub="Put-away, picking, replenishment and moves"
        />
        <Stat
          label="Stalled over 24h"
          value={qty(ex?.staleTasks?.length ?? 0)}
          tone={(ex?.staleTasks?.length ?? 0) > 0 ? "warn" : "neutral"}
          href="/warehouse"
        />
        <Stat
          label="Short picks"
          value={qty(ex?.shortPicks?.length ?? 0)}
        ...
        <Stat
          ...
          value={
            occupancy.data?.summary?.averageOccupancyPercent === null
              ? "Not measured"
              : `${occupancy.data?.summary?.averageOccupancyPercent ?? 0}%`
          }
          sub={`${occupancy.data?.summary?.empty ?? 0} empty bins`}
          href="/warehouse"
        />
      </div>
        ...
        <Card title="Work waiting, by type">
          {byType.length ? (
            ...
            <EmptyState
              ...
              body="No open put-away, pick or replenishment task."
            />
          )}
        </Card>
        <Card
          title="Bins over capacity"
          description="A bin holding more than it declares will misdirect the next put-away."
        >
            ...
            <Table head={["Bin", "Held", "Capacity", "Fullness"]}>
              {ex.overCapacityLocations.slice(0, 8).map((l: any) => (
                <tr key={l.id}>
                  ...
                  <td className="td">
                    <StatusBadge tone="out">{l.occupancyPercent}%</StatusBadge>
                  </td>
                </tr>
              ))}
            </Table>
    ...
    <>
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        <Stat
          ...
          value={qty(c.purchaseOrdersPending)}
          tone={c.purchaseOrdersPending > 0 ? "info" : "neutral"}
          href="/procurement"
        />
        <Stat
          label="Out of stock"
          value={qty(c.outOfStock)}
          tone={c.outOfStock > 0 ? "danger" : "neutral"}
          sub="Nothing on hand to sell or dispense"
          href="/inventory"
        />
        <Stat
          label="Below reorder point"
          value={qty(c.lowStock)}
          tone={c.lowStock > 0 ? "warn" : "neutral"}
          sub="Reorder recommendations are ready"
          href="/forecast"
        />
        <Stat
          label="Stock value"
          value={money(c.totalInventoryValue)}
          sub={`Turnover ${Number(c.stockTurnover ?? 0).toFixed(2)}×`}
          href="/accounting"
        />
      </div>
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        <Card
          ...
          description="Scored from delivery, rejection and short-shipment history, not from opinion."
        >
          ...
          {suppliers.data?.length ? (
            ...
            <EmptyState
              title="No supplier has been scored yet"
        ...
        <Card
          title="Slow movers"
          description="Stock that is not turning. Ordering more of it costs twice."
        >
          {data.charts.slowMovers?.length ? (
            <BarChart
              data={data.charts.slowMovers.slice(0, 8)}
              labelKey="name"
              valueKey="quantityMoved"
              format={(v) => qty(v)}
              tone="warn"
    ...
    <>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 xl:grid-cols-4">
        ...
        <Stat
          ...
          value={money(c.salesThisMonth)}
          href="/reports"
          sub={`${money(c.grossProfit)} gross profit`}
        />
        <Stat
          label="Gross margin"
          value={`${Number(c.grossMarginPct ?? 0).toFixed(1)}%`}
          href="/reports"
        />
        <Stat
          label="Stock turnover"
          value={`${Number(c.stockTurnover ?? 0).toFixed(2)}×`}
          href="/reports"
        />
        <Stat
          label="Out of stock"
        ...
        <Stat
          ...
          value={money(c.expiryValueAtRisk)}
          tone={c.nearExpiry > 0 ? "warn" : "neutral"}
          sub={`${qty(c.nearExpiry)} batch positions`}
          href="/inventory/expiry"
        />
        <Stat
          label="Open recalls"
          value={qty(c.openRecalls)}
          tone={c.openRecalls > 0 ? "danger" : "neutral"}
          sub={
            c.openExcursions
              ? `${c.openExcursions} cold-chain excursion(s)`
              : "Cold chain steady"
          }
          href="/recalls"
        />
...
function ExpiryExposure({ data }: { data: any }) {
  ...
  return (
    <Card
      ...
      description="Value that leaves the balance sheet if nothing is done."
    >
      {rows.length ? (
        <BarChart
          data={rows}
