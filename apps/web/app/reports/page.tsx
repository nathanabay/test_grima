    ...
    <Shell>
      ...
      <div className="grid gap-4 lg:grid-cols-4">
        <Card
          ...
          title={`${catalog.data?.length ?? 0} reports`}
        >
          {catalog.loading && <Loading />}
          <div className="max-h-[70vh] space-y-3 overflow-y-auto">
            {Object.entries(grouped).map(([group, reports]) => (
              <div key={group}>
                <div className="px-1 pb-1 text-[11px] font-semibold uppercase tracking-wider text-ink-subtle">
                  {group}
                </div>
                {reports.map((r) => (
                  <button
              ...
              {result.data && (
                <>
                  ...
                  {result.data.rows.length ? (
                    <>
                      <Table
                        head={result.data.columns.map((c: any) => c.label)}
                      >
                        {result.data.rows
                          .slice(0, 200)
                      ...
                      {result.data.rows.length > 200 && (
                        <p className="mt-2 text-xs text-ink-subtle">
                          Showing the first 200 of {result.data.rows.length}{" "}
                          rows. Export to see them all.
                        </p>
                      )}
                    </>
                  ) : (
