...
export default function ReportBuilderPage() {
  ...
  async function run() {
    ...
    setError(null);
    try {
      setResult(await api('/report-builder/run', { method: 'POST', body: definition() }));
    } catch (e: any) {
      setError(e.message);
      setResult(null);
    } finally {
  ...
  async function exportCsv() {
    ...
    try {
      ...
      const csv = await api<string>('/report-builder/export', { method: 'POST', body: definition() });
  ...
  async function runSaved(report: any) {
    ...
    setError(null);
    try {
      const out = await api<any>(`/report-builder/saved/${report.id}/run`, { method: 'POST', body: {} });
  ...
  return (
    <Shell>
      ...
      {catalogue.data && catalogue.data.length === 0 && (
        <Empty>You do not hold the read permission for any reportable data source.</Empty>
      )}
        ...
        <div className="space-y-5">
          <Card title="1 · Choose the data">
            <div className="grid gap-3 md:grid-cols-2">
              <label className="text-xs text-ink-muted">
                Data source
                <select className="input mt-1" value={sourceKey} onChange={(e) => selectSource(e.target.value)}>
              ...
              {source && <p className="self-end text-xs text-ink-muted">{source.description}</p>}
            </div>

            {source && (
              <div className="mt-4">
                <div className="text-xs font-medium text-ink">
                  Columns ({chosen.length ? `${chosen.length} chosen` : 'all available'})
                </div>
                  ...
                  {source.columns.map((c) => (
                    ...
                    >
                      <input
                        ...
                        onChange={(e) =>
                          setChosen((s) => (e.target.checked ? [...s, c.key] : s.filter((x) => x !== c.key)))
                        }
                      />
                      {c.label}
                      {!c.available && <Pill tone="warn">Needs {c.requires}</Pill>}
                    </label>
                  ))}
                </div>
              </div>
          ...
          {source && (
            <>
              <Card
                ...
                action={
                  <button
                    className="btn-ghost"
                    onClick={() =>
                      setFilters((f) => [...f, { field: source.columns[0].key, operator: 'eq', value: '', value2: '' }])
                    }
                  >
                    Add a condition
                  </button>
              ...
              >
                {filters.length === 0 ? (
                  <p className="text-xs text-ink-muted">No condition — every row the source returns is included.</p>
                ) : (
                  <div className="space-y-2">
                      ...
                      return (
                        <div key={i} className="flex flex-wrap items-center gap-2">
                          <select
                            ...
                            value={f.field}
                            onChange={(e) =>
                              setFilters((rows) => rows.map((r, j) => (j === i ? { ...r, field: e.target.value } : r)))
                            }
                          >
                            {source.columns.map((c) => (
                          ...
                          <select
                            ...
                            value={f.operator}
                            onChange={(e) =>
                              setFilters((rows) => rows.map((r, j) => (j === i ? { ...r, operator: e.target.value } : r)))
                            }
                          >
                            {OPERATORS.map((o) => (
                          ...
                          {(op?.values ?? 1) >= 1 && (
                            <input
                              ...
                              placeholder="Value"
                              onChange={(e) =>
                                setFilters((rows) => rows.map((r, j) => (j === i ? { ...r, value: e.target.value } : r)))
                              }
                            />
                          )}
                          {(op?.values ?? 0) === 2 && (
                            <input
                              ...
                              placeholder="and"
                              onChange={(e) =>
                                setFilters((rows) => rows.map((r, j) => (j === i ? { ...r, value2: e.target.value } : r)))
                              }
                            />
                          )}
                          <button
                    ...
                    <p className="text-xs text-ink-subtle">
                      All conditions must hold. A field the row does not carry never matches.
                    </p>
                  </div>
                )}
              ...
              <Card title="3 · Group and sort">
                <div className="grid gap-3 md:grid-cols-3">
                  ...
                  <label className="text-xs text-ink-muted">
                    Sort by
                    <select className="input mt-1" value={sortField} onChange={(e) => setSortField(e.target.value)}>
                  ...
                  <label className="text-xs text-ink-muted">
                    Direction
                    <select className="input mt-1" value={sortDir} onChange={(e) => setSortDir(e.target.value as any)}>
                ...
                {groupBy && (
                  <div className="mt-4">
                    <div className="text-xs font-medium text-ink">How to summarise each numeric column</div>
                    <div className="mt-2 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
                      {source.columns
                        .filter((c) => c.numeric && c.available)
                        .map((c) => (
                          <label key={c.key} className="flex items-center gap-2 text-xs text-ink-muted">
              ...
              <div className="flex flex-wrap gap-2">
                <button className="btn-primary" disabled={busy || !sourceKey} onClick={run}>
                  {busy ? 'Running…' : 'Run report'}
                </button>
                <button className="btn-ghost" disabled={busy || !result} onClick={exportCsv}>
                  Export CSV
                </button>
                <button className="btn-ghost" disabled={busy || !sourceKey} onClick={saveReport}>
                  Save definition
                </button>
              </div>
            </>
          ...
          {result && (
            <Card title={`${result.label} — result`}>
              <div className="mb-3 grid grid-cols-2 gap-3 sm:grid-cols-4">
                ...
                <Stat label="Grouped" value={result.grouped ? `By ${result.groupBy}` : 'No'} />
                <Stat
                  label="Complete"
                  value={result.truncated ? 'Truncated' : 'Yes'}
                  tone={result.truncated ? 'warn' : 'neutral'}
                  sub={result.truncated ? 'The row limit was reached — raise it to see the rest' : undefined}
                />
              </div>
              ...
              {result.withheldColumns?.length > 0 && (
                ...
                <div className="mb-3 rounded-md border border-warn/40 bg-warn-light p-3 text-xs text-warn">
                  {result.withheldColumns.length} column(s) were left out because you do not hold the permission they
                  need:{' '}
                  {result.withheldColumns.map((w: any) => `${w.key} (needs ${w.requires})`).join(', ')}.
                </div>
              )}
              ...
              (saved.data.length === 0 ? (
                ...
                <DataTable
                  ...
                  columns={[
                    ...
                    { key: 'dataSource', label: 'Source', value: (r: any) => r.dataSource },
                    { key: 'columns', label: 'Columns', numeric: true, align: 'right', value: (r: any) => r.columns.length },
                    {
                      key: 'isShared',
                      label: 'Shared',
                      value: (r: any) => (r.isShared ? 'Yes' : 'No'),
                      render: (r: any) => <Pill tone={r.isShared ? 'info' : 'neutral'}>{r.isShared ? 'Shared' : 'Private'}</Pill>,
                    },
                    {
                      key: 'actions',
                      label: '',
                      render: (r: any) => (
                        <div className="flex gap-2">
                          <button className="btn-ghost" disabled={busy} onClick={() => void runSaved(r)}>
                            Run
                          </button>
                          <button className="btn-ghost" disabled={busy} onClick={() => void deleteSaved(r)}>
                            Delete
                          </button>
                        </div>
                      ),
