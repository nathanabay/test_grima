...
export default function SettingsPage() {
  ...
  const config = useApi<{ settings: SettingRow[]; features: FlagRow[] }>('/admin/config', [version]);
  ...
  async function reset(row: SettingRow) {
    ...
    setBusy(true);
    try {
      await api(`/admin/config/${encodeURIComponent(row.key)}/reset`, { method: 'POST' });
  ...
  async function toggleFlag(flag: FlagRow, next: boolean) {
    ...
    setError(null);
    try {
      await api('/admin/config', { method: 'PATCH', body: { values: { [flag.key]: next } } });
  ...
  return (
    <Shell>
      <PageHeader
        ...
        action={
          <button className="btn-primary" disabled={!dirty.length || busy} onClick={save}>
      ...
      {config.data && (
        <div className="space-y-5">
          <Card
            title="Feature flags"
            action={<span className="text-xs text-ink-subtle">{config.data.features.length} flags</span>}
          >
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        ...
                        <div className="mt-0.5 text-xs text-ink-muted">{flag.description}</div>
                      </div>
                      <button
                    ...
                    {blocked && (
                      ...
                      <p className="mt-2 text-xs text-warn">{flag.unavailableReason}</p>
                    )}
                  </div>
                );
              })}
          ...
          <Card
            title="Settings"
            action={
              <select className="input w-56" value={group} onChange={(e) => setGroup(e.target.value)}>
                {groups.map((g) => (
                      ...
                      <div className="flex flex-wrap items-start justify-between gap-3">
                        <div className="min-w-0 flex-1">
                          ...
                          <div className="mt-1 flex flex-wrap items-center gap-2 text-[11px] text-ink-subtle">
                            ...
                            {row.min !== undefined && <span>&middot; min {row.min}</span>}
                        ...
                        <div className="flex w-full items-center gap-2 sm:w-72">
                          {row.type === 'boolean' ? (
                            ...
                            <input
                              ...
                              value={current}
                              placeholder={row.type.endsWith('[]') ? 'Comma separated' : ''}
                              onChange={(e) => setDraft((d) => ({ ...d, [row.key]: e.target.value }))}
                            />
                          )}
                          {row.isOverridden && (
                            <button className="btn-ghost" disabled={busy} onClick={() => reset(row)}>
                              Reset
                            </button>
                          )}
                        </div>
