...
interface Rule {
  ...
  conditions: { match?: string; conditions: { field: string; operator: string; value?: unknown }[] };
...
export default function AutomationPage() {
  ...
  async function runRule(rule: Rule) {
    const { confirmed } = await confirm({
      ...
      body: (
        <>
          This takes the rule&apos;s actions for real. Preview it first if you have not already —
          the last preview matched{' '}
          <strong>{preview?.matched ?? 'an unknown number of'}</strong> record(s).
        </>
      ),
    ...
    setError(null);
    try {
      const result: any = await api(`/automation/rules/${rule.id}/run`, { method: 'POST' });
      toast(
  ...
  async function toggle(rule: Rule) {
    ...
    const { confirmed } = await confirm({
      title: turningOff ? `Turn off “${rule.name}”?` : `Turn on “${rule.name}”?`,
      body: turningOff
    ...
    <Shell>
          ...
          <DataTable
            ...
            columns={[
              { key: 'name', label: 'Rule', value: (r) => r.name },
              {
                ...
                render: (r) => <Pill tone={r.isActive ? 'ok' : 'neutral'}>{r.isActive ? 'On' : 'Off'}</Pill>,
              },
              { key: 'triggerType', label: 'Trigger', value: (r) => r.triggerType },
              {
                key: 'actions',
                label: 'Actions',
              ...
              },
              {
                key: 'escalations',
                label: 'Escalates',
                value: (r) => (r.escalations.length ? `after ${r.escalations[0].afterHours}h` : 'no'),
                optional: true,
              },
              { key: 'priority', label: 'Priority', value: (r) => r.priority, numeric: true, optional: true },
              {
                key: 'cooldownHours',
                label: 'Cooldown',
      ...
      {selected && (
        <Card
          ...
          action={
            <div className="flex gap-2">
              <button className="btn-ghost" onClick={() => loadPreview(selected)} disabled={busy}>
                Refresh preview
              </button>
              <button className="btn-ghost" onClick={() => toggle(selected)}>
                {selected.isActive ? 'Turn off' : 'Turn on'}
              </button>
              <button className="btn-primary" onClick={() => runRule(selected)} disabled={busy || !selected.isActive}>
                Run now
              </button>
            </div>
          }
        >
          ...
          {selected.description && <p className="mb-3 text-sm text-ink-muted">{selected.description}</p>}

          {busy && !preview && <Loading label="Working out what this rule matches" />}
            ...
            <>
              <div className="mb-3 grid grid-cols-2 gap-3 sm:grid-cols-4">
                <div className="card p-3">
                  ...
                  <div className="text-lg font-semibold num">{preview.scanned}</div>
                </div>
                <div className="card p-3">
                  <div className="text-xs text-ink-muted">Would match</div>
                  <div className={`text-lg font-semibold num ${preview.matched ? 'text-warn' : ''}`}>
                    {preview.matched}
                  </div>
                </div>
                <div className="card p-3 sm:col-span-2">
                  <div className="text-xs text-ink-muted">Would do</div>
                  <div className="text-sm font-medium">{preview.wouldAct.join(', ') || 'nothing'}</div>
                </div>
              </div>
              ...
              {preview.samples.length === 0 ? (
                ...
                <div className="space-y-3">
                  {preview.samples.map((sample: any) => (
                    <div key={sample.subjectId} className="rounded-md border border-surface-border p-3">
                      {sample.preview.map((p: any, i: number) => (
                        <div key={i} className="mb-2">
                          <div className="text-sm font-medium text-ink">{p.title ?? p.type}</div>
                          {p.body && (
                            <div className="whitespace-pre-line text-sm text-ink-muted">{p.body}</div>
                          )}
                        </div>
                      ))}
                      <details className="text-xs text-ink-subtle">
                        <summary className="cursor-pointer">Why it matched</summary>
                        <ul className="mt-1 space-y-0.5">
                          {sample.conditionDetail.map((c: any, i: number) => (
                            <li key={i}>
                              {c.field} {c.operator} {JSON.stringify(c.expected)} — actual{' '}
                              {JSON.stringify(c.actual)}{' '}
                              <span className={c.matched ? 'text-ok' : 'text-danger'}>
                                {c.matched ? '✓' : '✗'}
                              </span>
                            </li>
                          ))}
                    ...
                    {preview.nearMisses.map((miss: any) => (
                      <li key={miss.subjectId}>
                        {miss.conditionDetail
                          ...
                          .map((c: any) => `${c.field} was ${JSON.stringify(c.actual)}`)
                          .join(', ')}
                      </li>
                    ))}
                  </ul>
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-2">
              ...
              <tbody>
                {runs.data.map((run: any) => (
                  <tr key={run.id}>
                    ...
                    <td className="td text-right num text-ink-subtle">{run.suppressed}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <Card
          title="Unresolved"
          action={<span className="text-xs text-ink-subtle">Subjects a rule acted on that are still open</span>}
        >
          {escalations.loading ? (
            <Skeleton rows={4} />
              ...
              <tbody>
                {escalations.data.slice(0, 20).map((e: any) => (
                  <tr key={e.id}>
                    ...
                    <td className="td">
                      <Pill tone={e.level > 0 ? 'danger' : 'warn'}>{e.status}</Pill>
                    </td>
                    <td className="td text-ink-muted">
                      {e.nextDueAt ? shortDate(e.nextDueAt) : 'no further steps'}
                    </td>
                  </tr>
                ))}
              </tbody>
