...
function ProductBody() {
  ...
  const stock = useApi<any>(id ? `/inventory/products/${id}/stock` : null, [
    id,
  ]);
  ...
  const ingredients = useApi<any>(
    id && tab === "overview" ? `/products/${id}/ingredients` : null,
    [id, tab],
  );
  const relations = useApi<any>(
    id && tab === "overview" ? `/products/${id}/relations` : null,
    [id, tab],
  );
  const attributes = useApi<any>(
    id && tab === "overview" ? `/products/${id}/attributes` : null,
    [id, tab],
  );
  const prices = useApi<any>(
    id && tab === "pricing" ? `/products/${id}/price-history` : null,
    [id, tab],
  );

  if (product.loading) return <Loading label="Loading product" />;
  if (product.error)
    return <ErrorState message={product.error} onRetry={product.refresh} />;
  ...
  return (
    <>
      <PageHeader
        ...
        title={[p.brandName || p.genericName, p.strength]
          .filter(Boolean)
          .join(" ")}
        subtitle={
        ...
        action={
          <div className="flex flex-wrap items-center gap-1.5">
            ...
            {p.requiresPrescription && (
              <StatusBadge tone="info">Prescription only</StatusBadge>
            )}
            {p.isControlled && (
              <StatusBadge tone="controlled">
                Controlled
                {p.controlledSchedule ? ` · ${p.controlledSchedule}` : ""}
              </StatusBadge>
            )}
            {p.isColdChain && <StatusBadge tone="cold">Cold chain</StatusBadge>}
            {p.isHighAlert && <StatusBadge tone="out">High alert</StatusBadge>}
            {p.isLookAlikeSoundAlike && (
              <StatusBadge tone="near">Look-alike</StatusBadge>
            )}
          </div>
        }
      />
      ...
      <div className="mb-4 grid grid-cols-2 gap-3 sm:grid-cols-3 xl:grid-cols-6">
        <Stat
          label="Available"
          value={qty(totals.available)}
          tone={totals.available > 0 ? "ok" : "danger"}
          sub={`of ${qty(totals.onHand)} on hand`}
        />
        <Stat
          label="Reserved"
          value={qty(totals.reserved)}
          sub="Held for open documents"
        />
        <Stat
          label="Quarantined"
          value={qty(totals.quarantined)}
          tone={totals.quarantined > 0 ? "warn" : "neutral"}
        />
        <Stat
          label="Recalled"
          value={qty(totals.recalled)}
          tone={totals.recalled > 0 ? "danger" : "neutral"}
        />
        <Stat
          label="Expired"
          value={qty(totals.expired)}
          tone={totals.expired > 0 ? "danger" : "neutral"}
        />
        <Stat
          label="Retail price"
          value={money(p.retailPrice)}
          ...
          <Card title="Identity and classification">
            <dl className="grid grid-cols-2 gap-x-4 gap-y-2 text-body">
              ...
              <Detail
                ...
                value={
                  p.registrationExpiry ? shortDate(p.registrationExpiry) : null
                }
              />
          ...
          <Card title="Storage and handling">
            <dl className="grid grid-cols-2 gap-x-4 gap-y-2 text-body">
              <Detail
                ...
                value={p.storageCondition?.replace(/_/g, " ")}
              />
              <Detail
                label="Temperature range"
                value={
                  p.minTempC !== null && p.maxTempC !== null
                    ? `${p.minTempC}–${p.maxTempC} °C`
                    : null
                }
              />
              <Detail
                ...
                value={
                  p.minHumidityPercent !== null && p.maxHumidityPercent !== null
                    ? `${p.minHumidityPercent}–${p.maxHumidityPercent} %`
                    : null
                }
              />
              <Detail
                label="Max excursion"
                value={
                  p.maxExcursionMinutes ? `${p.maxExcursionMinutes} min` : null
                }
              />
              <Detail
                label="Minimum shelf life on receipt"
                value={`${p.minShelfLifeDaysOnReceipt} days`}
              />
              <Detail
                label="Sale classification"
                value={p.saleClassification?.replace(/_/g, " ")}
              />
            </dl>
            <div className="mt-3 flex flex-wrap gap-1.5">
              ...
              {p.isNarcotic && (
                <StatusBadge tone="controlled">Narcotic</StatusBadge>
              )}
              {p.lightSensitive && (
                <StatusBadge tone="near">Light sensitive</StatusBadge>
              )}
              ...
              {p.isVeterinary && (
                <StatusBadge tone="info">Veterinary</StatusBadge>
              )}
              {p.isPediatric && (
                <StatusBadge tone="info">Paediatric</StatusBadge>
              )}
            </div>
          </Card>
          ...
          <Card title="Active ingredients">
            ...
            {(p.pregnancyInfo || p.lactationInfo) && (
              <div className="mt-3 space-y-1 border-t border-border pt-3 text-small text-ink-muted">
                {p.pregnancyInfo && (
                  <p>
                    <span className="font-medium text-ink">Pregnancy: </span>
                    {p.pregnancyInfo}
                  </p>
                )}
                {p.lactationInfo && (
                  <p>
                    <span className="font-medium text-ink">Lactation: </span>
                    {p.lactationInfo}
                  </p>
                )}
                <p className="text-caption text-ink-subtle">
                  Free text from the approved product information. Nothing here
                  is generated.
                </p>
              </div>
            )}
            ...
            {relations.data?.length ? (
              <ul className="space-y-1.5 text-body">
                {relations.data.map((r: any) => (
                  ...
                  >
                    <Link
                      href={`/products/${r.relatedProductId}`}
                      className="text-brand-dark hover:underline"
                    >
                      {r.relatedProduct?.genericName ?? r.relatedProductId}
                    </Link>
                    <StatusBadge tone="info">
                      {r.relationType.replace(/_/g, " ").toLowerCase()}
                    </StatusBadge>
                  </li>
                ))}
              </ul>
            ...
            {attributes.data?.length > 0 && (
              <dl className="mt-3 grid grid-cols-2 gap-x-4 gap-y-1 border-t border-border pt-3 text-body">
                {attributes.data.map((a: any) => (
                  <Detail
                    key={a.id}
                    label={a.definition?.label ?? a.definitionId}
                    value={a.value}
                  />
                ))}
              </dl>
      ...
      {tab === "inventory" && (
        <div className="space-y-4">
          <Card
            ...
            description="Every batch, in every warehouse and bin, with the state that decides whether it can be picked."
          >
            ...
            {positions.length === 0 ? (
              ...
              <DataTable
                ...
                getKey={(b: any) =>
                  b.id ??
                  `${b.batchId}-${b.warehouseId}-${b.locationId ?? "wh"}`
                }
                ...
                columns={[
                  ...
                  },
                  {
                    ...
                    label: "Expiry",
                    value: (b: any) =>
                      b.batch?.expiryDate ?? b.expiryDate ?? "",
                    render: (b: any) => {
                  ...
                  },
                  {
                    ...
                    render: (b: any) => (
                      <StatusBadge status={b.batch?.status ?? b.batchStatus} />
                    ),
                  },
                  {
                    key: "warehouse",
                    label: "Warehouse",
                    value: (b: any) => b.warehouse?.name ?? "",
                  },
                  {
                    key: "location",
                    label: "Bin",
                    optional: true,
                    value: (b: any) => b.location?.code ?? "",
                  },
                  {
                    key: "onHand",
                    label: "On hand",
                  ...
                  },
                  {
                    ...
                    render: (b: any) => (
                      <QuantityCell
                        value={Number(b.onHand ?? 0) - Number(b.reserved ?? 0)}
                      />
                    ),
                  },
                ]}
          ...
          <Card
            title="Across branches"
            description="Where else this medicine is held, so a shortage here can be met from there."
          >
            ...
            {branches.data?.length ? (
              <DataTable
                rows={branches.data}
                getKey={(b: any) =>
                  `${b.branchId}-${b.warehouseId}-${b.batchNumber}`
                }
                pageSize={10}
                ...
                columns={[
                  ...
                  },
                  {
                    key: "warehouse",
                    label: "Warehouse",
                    value: (b: any) => b.warehouseName,
                  },
                  {
                    key: "batch",
                    label: "Batch",
                    value: (b: any) => b.batchNumber,
                  },
                  {
                    key: "expiry",
                    label: "Expiry",
                    value: (b: any) => b.expiryDate,
      ...
      {tab === "planning" && (
        <div className="grid gap-4 xl:grid-cols-2">
          <Card
            title="Replenishment levels"
            description="What the reorder engine reads. These are configuration, not observations."
          >
            <dl className="grid grid-cols-2 gap-x-4 gap-y-2 text-body">
              ...
              <Detail
                ...
                value={p.purchaseMultiple ? qty(p.purchaseMultiple) : "None"}
              />
              <Detail
                label="Maximum dispense"
                value={p.maxDispenseQty ? qty(p.maxDispenseQty) : "No cap"}
              />
            </dl>
            {p.procurementRestricted && (
              <p className="mt-3 rounded border border-warn/30 bg-warn-light px-3 py-2 text-small text-warn">
                Procurement of this product is restricted. It cannot be ordered
                outside the emergency workflow.
              </p>
            )}
          </Card>
          <Card
            title="Forecast"
            description="Demand projected from the movement history, with its method stated."
          >
            <p className="text-small text-ink-muted">
              Open{" "}
              <Link href="/forecast" className="text-brand-dark underline">
                Forecasting
              </Link>{" "}
              for the projection, the method that produced it and its confidence
              range. Nothing here presents a prediction as a certainty.
            </p>
          </Card>
        </div>
      )}
      ...
      {tab === "pricing" && (
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            ...
            <Stat
              label="Insurance"
              value={p.insurancePrice ? money(p.insurancePrice) : "—"}
            />
            <Stat
              label="Average cost"
              value={money(p.averageCost)}
                ...
                columns={[
                  {
                    key: "changedAt",
                    label: "When",
                    value: (h: any) => h.changedAt ?? h.createdAt,
                    render: (h: any) => shortDate(h.changedAt ?? h.createdAt),
                  },
                  {
                    key: "priceType",
                    label: "Price",
                    value: (h: any) => h.priceType ?? "RETAIL",
                  },
                  {
                    key: "oldPrice",
                    label: "From",
                    numeric: true,
                    align: "right",
                    value: (h: any) => Number(h.oldPrice ?? 0),
                    render: (h: any) => money(h.oldPrice),
                  },
                  {
                    key: "newPrice",
                    label: "To",
                    numeric: true,
                    align: "right",
                    value: (h: any) => Number(h.newPrice ?? 0),
                    render: (h: any) => money(h.newPrice),
                  },
                  {
                    key: "reason",
                    label: "Reason",
                    value: (h: any) => h.reason ?? "",
                  },
                ]}
              />
            ) : (
              <EmptyState
                title="No price change recorded"
      ...
      {tab === "activity" && (
        <Card
          title="Everything that has happened to this product"
...
function Detail({
  ...
  return (
    <>
      ...
      <dd className={`text-ink ${mono ? "num" : ""}`}>
