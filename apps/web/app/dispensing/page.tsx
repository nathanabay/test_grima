...
export default function DispensingPage() {
  ...
  async function review(id: string, decision: "APPROVE" | "REJECT") {
    ...
    setMessage(null);
    const reason =
      decision === "REJECT" ? window.prompt("Rejection reason:") : undefined;
    if (decision === "REJECT" && !reason) return;
    setBusy(true);
    try {
      await api(`/prescriptions/${id}/review`, {
        method: "POST",
        body: { decision, reason },
      });
      setMessage(
        `Prescription ${decision === "APPROVE" ? "approved" : "rejected"}.`,
      );
      list.refresh();
      setSelected(null);
    } catch (e: any) {
  ...
  async function dispense(prescription: any) {
    ...
    const branch =
      branches.find((b) => b.id === prescription.branchId) ?? branches[0];
    ...
    <Shell>
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        ...
        <Card title="Selected prescription">
          {!selected && (
            <Empty>Choose a prescription to review or dispense.</Empty>
          )}
          {selected && (
            <div className="space-y-3 text-sm">
                ...
                {selected.items.map((i: any) => (
                  <div key={i.id} className="rounded-md bg-surface-sunken p-2">
                    ...
                    <div className="text-xs text-ink-muted">
                      Prescribed {qty(i.prescribedQty)} · dispensed{" "}
                      {qty(i.dispensedQty)}
              ...
              <div className="flex flex-wrap gap-2">
                {canReview &&
                  ["NEW", "UNDER_REVIEW"].includes(selected.status) && (
                    <>
                      <button
                ...
                {canDispense &&
                  ["APPROVED", "PARTIALLY_DISPENSED"].includes(
                    selected.status,
                  ) && (
                    <button
                      className="btn-primary"
                      disabled={busy}
                      onClick={() => dispense(selected)}
                    >
