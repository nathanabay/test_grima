        ...
        <div className="mb-4 grid grid-cols-2 gap-3 sm:grid-cols-6">
          {[
            ...
            <div key={String(label)} className="card p-3">
              ...
              <div
                className={`text-lg font-semibold num ${i === 4 ? "text-danger" : ""}`}
              >
                {money(value)}
              </div>
            </div>
              ...
              {list.rows.map((inv: any) => (
                ...
                >
                  ...
                  <div className="text-xs text-ink-subtle">
                    {inv.supplier.companyName} · {money(inv.grandTotal)} · due{" "}
                    {shortDate(inv.dueDate)}
                  </div>
                  <div className="mt-1">
                    <Pill tone={MATCH_TONE[inv.matchStatus]}>
              ...
              title={
                <span>
                  ...
                  <span className="ml-2 text-xs font-normal text-ink-subtle">
                    {detail.data.internalNo} ·{" "}
                    {detail.data.supplier.companyName}
                  </span>
                </span>
              }
            >
              <div className="mb-3 flex flex-wrap items-center gap-2">
                ...
                <span className="text-sm text-ink-muted">
                  {money(detail.data.amountPaid)} paid of{" "}
                  {money(detail.data.grandTotal)}
                </span>
              </div>
              ...
              {detail.data.payments.length > 0 && (
                <div className="mt-4">
                  ...
                  <Table
                    head={["Payment", "When", "Method", "Reference", "Amount"]}
                  >
              ...
              <div className="mt-4 flex flex-wrap gap-2 border-t border-surface-border pt-3">
                <button
                  ...
                  onClick={() =>
                    act(
                      `/supplier-invoices/${detail.data.id}/match`,
                      {},
                      "Re-matched",
                    )
                  }
                ...
                {["MATCHED", "DISPUTED", "SUBMITTED"].includes(
                  detail.data.status,
                ) && (
                  <button
