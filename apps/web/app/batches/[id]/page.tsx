...
function BatchBody() {
  ...
  const trace = useApi<any>(
    id && tab === "trace" ? `/batches/${id}/trace` : null,
    [id, tab, version],
  );

  if (batch.loading) return <Loading label="Loading batch" />;
  if (batch.error)
    return <ErrorState message={batch.error} onRetry={batch.refresh} />;
  ...
  return (
    <>
      ...
      {b.recallLinks?.length > 0 && (
        <div className="mb-4 rounded border border-st-recall/40 bg-st-recall/10 px-4 py-3">
          <p className="text-body font-medium text-st-recall">
            ...
            {b.recallLinks.length === 1 ? "" : "s"}. It cannot be sold or
            dispensed.
          </p>
      ...
      <div className="mb-4 grid grid-cols-2 gap-3 sm:grid-cols-3 xl:grid-cols-6">
        ...
        <Stat
          label="Available"
          value={qty(Math.max(0, onHand - reserved))}
          tone={["AVAILABLE", "RELEASED"].includes(b.status) ? "ok" : "danger"}
          sub={
            ["AVAILABLE", "RELEASED"].includes(b.status)
              ? "Allocatable"
              : `${b.status.toLowerCase()} — not allocatable`
          }
        />
        <Stat
          label="Received"
          value={qty(b.receivedQuantity)}
          sub={shortDate(b.receivedDate)}
        />
        <Stat
          label="Days remaining"
          value={days < 0 ? `${Math.abs(days)} past` : days}
          tone={days < 0 ? "danger" : days <= 30 ? "warn" : "neutral"}
          sub={shortDate(b.expiryDate)}
        />
        <Stat
          label="Value at cost"
          value={money(value)}
          sub={`${money(b.purchaseCost)} per unit`}
        />
      </div>
          ...
          <Card title="Batch identity">
            <dl className="grid grid-cols-2 gap-x-4 gap-y-2 text-body">
              ...
              <D
                label="Manufactured"
                value={
                  b.manufacturingDate ? shortDate(b.manufacturingDate) : null
                }
              />
          ...
          <Card title="Quality state">
            <dl className="grid grid-cols-2 gap-x-4 gap-y-2 text-body">
              ...
              <D
                ...
                value={b.quarantineReason?.replace(/_/g, " ")}
              />
              <D
                label="Released at"
                value={b.releasedAt ? shortDate(b.releasedAt) : null}
              />
            </dl>
            {b.qualityNotes && (
              ...
              <div className="mt-3 border-t border-border pt-3">
                ...
                {b.parentBatch && (
                  <p className="text-small">
                    Repacked from{" "}
                    <Link
                      href={`/batches/${b.parentBatch.id}`}
                      className="text-brand-dark underline num"
                    >
      ...
      {tab === "positions" && (
        <Card
          title="Where this batch is"
          description="Every warehouse and bin holding it right now."
        >
          {positions.length === 0 ? (
            <EmptyState
              title="No stock of this batch remains"
              ...
              columns={[
                {
                  key: "warehouse",
                  label: "Warehouse",
                  value: (p: any) => p.warehouse?.name ?? "",
                },
                {
                  key: "location",
                  label: "Bin",
                  value: (p: any) => p.location?.code ?? "—",
                },
                {
                  key: "onHand",
                  label: "On hand",
                  numeric: true,
                  align: "right",
                  value: (p: any) => Number(p.onHand),
                  render: (p: any) => <QuantityCell value={p.onHand} />,
                },
                {
                  key: "reserved",
                  label: "Reserved",
                  numeric: true,
                  align: "right",
                  value: (p: any) => Number(p.reserved),
                  render: (p: any) => <QuantityCell value={p.reserved} />,
                },
                {
                  key: "available",
                  label: "Available",
                  ...
                  render: (p: any) => (
                    <QuantityCell
                      value={Number(p.onHand) - Number(p.reserved)}
                    />
                  ),
                },
                {
                  ...
                  render: (p: any) =>
                    p.lastMovementAt ? shortDate(p.lastMovementAt) : "—",
                },
              ]}
            />
          )}
      ...
      {tab === "movements" && (
        <Card
          ...
          description="Append-only. Every row is a ledger entry that cannot be edited."
        >
            ...
            <DataTable
              ...
              columns={[
                ...
                },
                {
                  ...
                  render: (t: any) =>
                    Number(t.quantityIn) ? (
                      <QuantityCell value={t.quantityIn} />
                    ) : (
                      ""
                    ),
                },
                {
                  ...
                  render: (t: any) =>
                    Number(t.quantityOut) ? (
                      <QuantityCell value={t.quantityOut} />
                    ) : (
                      ""
                    ),
                },
                ...
                },
                {
                  key: "referenceNo",
                  label: "Document",
                  value: (t: any) => t.referenceNo ?? "",
                },
                {
                  key: "reason",
                  label: "Reason",
                  optional: true,
                  value: (t: any) => t.reason ?? "",
                },
              ]}
            />
          ) : (
      ...
      {tab === "trace" && (
        <div className="space-y-4">
          ...
          >
            ...
            {trace.error && (
              <ErrorState message={trace.error} onRetry={trace.refresh} />
            )}
            {trace.data && (
              <>
                <div className="mb-3 grid grid-cols-2 gap-3 sm:grid-cols-4">
                  <Stat
                    ...
                    value={trace.data.recipientSummary.dispensings}
                  />
                  <Stat
                    label="Sold over the counter"
                    value={trace.data.recipientSummary.sales}
                  />
                  <Stat
                    label="Contactable"
                    value={trace.data.recipientSummary.contactable}
                    tone="ok"
                    sub="Have a patient record"
                  />
                  <Stat
                    label="Un-contactable"
                    value={trace.data.recipientSummary.anonymousSales}
                    tone={
                      trace.data.recipientSummary.anonymousSales > 0
                        ? "warn"
                        : "neutral"
                    }
                      ...
                      columns={[
                        {
                          key: "branch",
                          label: "Branch",
                          value: (l: any) => l.branchName,
                        },
                        {
                          key: "warehouse",
                          label: "Warehouse",
                          value: (l: any) => l.warehouseName,
                        },
                        {
                          key: "onHand",
                          label: "On hand",
                ...
                {recipients.length ? (
                  <>
                    <p className="label">
                      Who received medicine from this batch
                    </p>
                    <DataTable
                      rows={recipients}
                      getKey={(r) => r.key}
                      ...
                      columns={[
                        {
                          ...
                          render: (r) => (
                            <StatusBadge
                              tone={
                                r.route === "Dispensed" ? "info" : "neutral"
                              }
                            >
                              {r.route}
                            </StatusBadge>
                  ...
                  <EmptyState
                    title="Nothing from this batch has left the pharmacy"
