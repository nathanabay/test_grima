                  ...
                  <td className="td">
                    ...
                    {s.calibrationStatus !== 'VALID' && (
                      <div className="mt-1 text-xs text-danger">
                        {s.calibrationStatus === 'OVERDUE' ? 'Calibration lapsed' : 'Never calibrated'}
                      </div>
                    )}
                  </td>
                </tr>
...
function Equipment({ onChanged }: { onChanged: () => void }) {
  ...
  return (
    <>
      ...
      >
        <div className="p-4">
          {due.error && <ErrorState message={due.error} onRetry={due.refresh} />}
          {due.loading && !due.data && <Loading />}

          {due.data && (
            <>
              <div className="mb-3 grid grid-cols-2 gap-3 sm:grid-cols-3">
                <Stat label="Active sensors" value={due.data.activeSensors} sub="Reporting equipment" />
                <Stat label="Needing attention" value={rows.length}
                  tone={rows.length ? 'warn' : 'ok'} sub="Calibration or service" />
                <Stat label="No valid certificate" value={rows.filter((r) => r.neverCalibrated || r.calibrationOverdue).length}
                  tone={rows.some((r) => r.neverCalibrated || r.calibrationOverdue) ? 'danger' : 'ok'}
              ...
              {rows.length === 0 ? (
                ...
                <DataTable
                  ...
                  columns={[
                    ...
                    { key: 'warehouse', label: 'Warehouse', value: (r: any) => r.warehouse },
                    { key: 'calibrationDueAt', label: 'Calibration due',
                      value: (r: any) => r.calibrationDueAt ?? '',
                      render: (r: any) =>
                        r.neverCalibrated
                          ? <span className="text-danger">never calibrated</span>
                          : shortDate(r.calibrationDueAt) },
                    { key: 'nextMaintenanceAt', label: 'Service due', optional: true,
                      value: (r: any) => r.nextMaintenanceAt ?? '',
                      render: (r: any) => (r.nextMaintenanceAt ? shortDate(r.nextMaintenanceAt) : 'not scheduled') },
                  ]}
                />
              )}
            </>
...
function EquipmentDrawer({
  ...
  const [version, setVersion] = useState(0);
  const [form, setForm] = useState<'calibration' | 'maintenance'>('calibration');
  ...
  const detail = useApi<any>(sensor ? `/cold-chain/equipment/${sensor.sensorId}` : null, [sensor?.sensorId, version]);
    ...
    try {
      if (form === 'calibration') {
        await api(`/cold-chain/equipment/${sensor.sensorId}/calibrations`, {
          method: 'POST',
          body: { result, certificateNo: certificateNo || undefined, performedBy: performedBy || undefined, notes: notes || undefined },
        });
      } else {
  ...
  return (
    <Drawer
      ...
      description={data ? `Calibration ${data.calibrationStatus.replace(/_/g, ' ').toLowerCase()}` : undefined}
    >
      ...
      {data && (
        <div className="space-y-4">
          <div className="flex flex-wrap items-center gap-2">
            <StatusBadge status={data.calibrationStatus === 'VALID' ? 'APPROVED' : 'BLOCKED'} />
            <span className="text-small text-ink-muted">
              {data.lastCalibratedAt ? `Last calibrated ${shortDate(data.lastCalibratedAt)}` : 'No certificate on file'}
              {data.calibrationDueAt ? ` · due ${shortDate(data.calibrationDueAt)}` : ''}
            </span>
          </div>
            ...
            <Panel title="Record">
              ...
              {form === 'calibration' ? (
                <div className="space-y-3">
                  <Field label="Result" hint="A FAIL is recorded and does not extend the due date — the instrument is not calibrated.">
                    <select className="input" value={result} onChange={(e) => setResult(e.target.value)}>
                      {['PASS', 'ADJUSTED', 'FAIL'].map((r) => <option key={r} value={r}>{r}</option>)}
                    </select>
                  </Field>
                  <Field label="Certificate number">
                    <input className="input" value={certificateNo} onChange={(e) => setCertificateNo(e.target.value)} />
                  </Field>
                  <Field label="Performed by">
                    <input className="input" value={performedBy} onChange={(e) => setPerformedBy(e.target.value)} />
                  </Field>
                  <Field label="Notes">
                    <textarea className="input min-h-[5rem]" value={notes} onChange={(e) => setNotes(e.target.value)} />
                  </Field>
                </div>
              ) : (
                <div className="space-y-3">
                  <Field label="Work type">
                    <select className="input" value={workType} onChange={(e) => setWorkType(e.target.value)}>
                      {['PREVENTIVE', 'CORRECTIVE', 'BATTERY', 'REPLACEMENT', 'INSPECTION'].map((w) => (
                        <option key={w} value={w}>{w}</option>
                      ))}
                    </select>
                  </Field>
                  <Field label="What was done" required hint="A record with no description proves nothing.">
                    <textarea className="input min-h-[5rem]" value={description} onChange={(e) => setDescription(e.target.value)} />
                  </Field>
                  <Field label="Performed by">
                    <input className="input" value={performedBy} onChange={(e) => setPerformedBy(e.target.value)} />
                  </Field>
              ...
              <button className="btn-primary btn-sm mt-3" disabled={busy || (form === 'maintenance' && !description.trim())}
                onClick={submit}>
          ...
          <Panel title={`Calibration history (${data.calibrations.length})`} padded={false}>
            {data.calibrations.length === 0 ? (
              <div className="p-4"><EmptyState title="No certificates on file" body="Readings from this sensor cannot be relied on until it is calibrated." /></div>
            ) : (
              <ul className="divide-y divide-border">
                {data.calibrations.map((c: any) => (
                  <li key={c.id} className="px-4 py-2.5 text-small">
                    <div className="flex items-center justify-between gap-2">
                      <span className={c.result === 'FAIL' ? 'font-medium text-danger' : 'text-ink'}>{c.result}</span>
                      <span className="text-caption text-ink-subtle">{shortDate(c.calibratedAt)}</span>
                    </div>
                    <div className="text-caption text-ink-muted">
                      Valid until {shortDate(c.validUntil)}
                      {c.certificateNo ? ` · certificate ${c.certificateNo}` : ''}
          ...
          <Panel title={`Service history (${data.maintenance.length})`} padded={false}>
            {data.maintenance.length === 0 ? (
              <div className="p-4"><EmptyState title="No service recorded" body="Preventive service, repairs and battery changes appear here." /></div>
            ) : (
              <ul className="divide-y divide-border">
                {data.maintenance.map((m: any) => (
                  <li key={m.id} className="px-4 py-2.5 text-small">
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <span className="text-caption text-ink-subtle">{shortDate(m.performedAt)}</span>
                    </div>
                    <div className="text-caption text-ink-muted">{m.description}</div>
                  </li>
                ))}
              </ul>
            )}
