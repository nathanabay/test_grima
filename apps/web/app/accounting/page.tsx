...
export default function AccountingPage() {
  ...
  const accounts = useApi<any[]>(tab === 'accounts' ? '/accounting/accounts?includeInactive=true' : null, [version]);
  const mapping = useApi<any[]>('/accounting/accounts/mapping-health', [version]);
  const journal = useApi<any>(tab === 'journal' ? '/accounting/journal?pageSize=100' : null, [version]);
  const entry = useApi<any>(selectedEntry ? `/accounting/journal/${selectedEntry}` : null, [selectedEntry, version]);
  ...
  const valuation = useApi<any>(tab === 'valuation' ? '/accounting/valuation' : null, [version]);
  const reconciliation = useApi<any>(tab === 'valuation' ? '/accounting/valuation/reconciliation' : null, [version]);
  const unposted = useApi<any>('/accounting/unposted?limit=50', [version]);
  const notes = useApi<any[]>(tab === 'notes' ? '/accounting/notes' : null, [version]);
  const periods = useApi<any[]>(tab === 'periods' ? '/accounting/periods' : null, [version]);
  ...
  async function reverse(e: any) {
    ...
    if (!confirmed) return;
    await act(`/accounting/journal/${e.id}/reverse`, { reason }, `${e.entryNo} reversed`);
  }
  ...
  async function postPending() {
    ...
    setError(null);
    try {
      const result = await api<any>('/accounting/post-pending', { method: 'POST', body: { limit: 200 } });
      const posted = result.movements + result.sales + result.invoices + result.payments;
      toast(
  ...
  async function ensureDefaults() {
    ...
    if (!confirmed) return;
    await act('/accounting/accounts/ensure-defaults', undefined, 'Default accounts checked');
  }
  ...
  async function issueNote(note: any) {
    ...
    if (!confirmed) return;
    await act(`/accounting/notes/${note.id}/issue`, undefined, `${note.noteNo} issued`);
  }
  ...
  async function reopenPeriod(p: any) {
    ...
    if (!confirmed) return;
    await act(`/accounting/periods/${p.id}/reopen`, { reason }, 'Period reopened');
  }
    ...
    <Shell>
      ...
      {unmapped.length > 0 && (
        <div className="mb-4 rounded-md border border-warn/40 bg-warn-light p-3 text-sm text-warn">
          <strong>{unmapped.length} system account(s) are not mapped.</strong> Until they are, the documents that need
          them cannot be posted:{' '}
          {unmapped.map((m) => m.systemKey).join(', ')}.
          <button className="btn-ghost ml-3" disabled={busy} onClick={ensureDefaults}>
      ...
      {unposted.data?.total > 0 && (
        <div className="mb-4 rounded-md border border-info/40 bg-info-light p-3 text-sm text-info">
          <strong>{unposted.data.total} document(s) have no journal entry.</strong>{' '}
          {unposted.data.movements.length} stock movement(s) and {unposted.data.sales.length} sale(s).
          <button className="btn-ghost ml-3" disabled={busy} onClick={postPending}>
            Post them now
          </button>
        </div>
      )}
        ...
        <div className="space-y-5">
            ...
            {accounts.data && (
              <DataTable
                ...
                columns={[
                  ...
                  { key: 'type', label: 'Type', value: (a: any) => a.type, render: (a: any) => <Pill tone="neutral">{a.type}</Pill> },
                  {
                    key: 'systemKey',
                    label: 'System role',
                    value: (a: any) => a.systemKey ?? '',
                    render: (a: any) => (a.systemKey ? <Pill tone="info">{a.systemKey}</Pill> : '—'),
                  },
                  { key: 'currency', label: 'Currency', optional: true, value: (a: any) => a.currency ?? '' },
                  {
                    key: 'active',
                    label: 'Status',
                    value: (a: any) => (a.isActive ? 'Active' : 'Inactive'),
                    render: (a: any) => <Pill tone={a.isActive ? 'ok' : 'neutral'}>{a.isActive ? 'Active' : 'Inactive'}</Pill>,
                  },
                ]}
              />
            )}
          ...
          <Card title="System account mapping">
            <p className="mb-3 text-xs text-ink-muted">
              Postings look accounts up by role, not by code, so a pharmacy can number its chart however it likes. A role
              with nothing mapped blocks the documents that need it, and says which.
            </p>
            {mapping.data && (
              <DataTable
                rows={mapping.data}
                ...
                columns={[
                  { key: 'systemKey', label: 'Role', value: (m: any) => m.systemKey },
                  { key: 'code', label: 'Account', value: (m: any) => (m.code ? `${m.code} ${m.name}` : '') , render: (m: any) => (m.code ? `${m.code} — ${m.name}` : <span className="text-danger">Not mapped</span>) },
                  {
                    key: 'state',
                    label: 'State',
                    value: (m: any) => (m.problem ? 'Problem' : 'OK'),
                    render: (m: any) => (m.problem ? <Pill tone="danger">Problem</Pill> : <Pill tone="ok">Mapped</Pill>),
                  },
                  { key: 'problem', label: 'Detail', value: (m: any) => m.problem ?? '', render: (m: any) => <span className="text-xs text-ink-muted">{m.problem ?? '—'}</span> },
                ]}
              />
            )}
          </Card>
        ...
        <div className="space-y-5">
          <Card title="Journal entries">
            ...
            {journal.data && (
              <DataTable
                ...
                onRowClick={(e: any) => setSelectedEntry((s) => (s === e.id ? null : e.id))}
                empty="No journal entry has been posted."
                columns={[
                  { key: 'entryNo', label: 'Entry', value: (e: any) => e.entryNo },
                  { key: 'entryDate', label: 'Date', value: (e: any) => e.entryDate, render: (e: any) => shortDate(e.entryDate) },
                  { key: 'description', label: 'Description', value: (e: any) => e.description },
                  { key: 'sourceType', label: 'Source', value: (e: any) => e.sourceType },
                  {
                    key: 'totalDebit',
                    label: 'Amount',
                    numeric: true,
                  ...
                  },
                  {
                    ...
                    render: (e: any) => <Pill tone={STATUS_TONE[e.status] ?? 'neutral'}>{e.status}</Pill>,
                  },
                  {
                    key: 'reverse',
                    label: '',
          ...
          {selectedEntry && entry.data && (
            <Card
              ...
              action={
                <button className="btn-ghost" onClick={() => setSelectedEntry(null)}>
                  Close
                </button>
              }
            >
              ...
              {entry.data.reversals?.length > 0 && (
                <p className="mb-3 text-xs text-danger">
                  Reversed by {entry.data.reversals.map((r: any) => r.entryNo).join(', ')}.
                </p>
              )}
              <DataTable
                ...
                columns={[
                  { key: 'lineNumber', label: '#', numeric: true, align: 'right', value: (l: any) => l.lineNumber },
                  { key: 'account', label: 'Account', value: (l: any) => `${l.account?.code ?? ''} ${l.account?.name ?? ''}` },
                  {
                    key: 'debit',
                    label: 'Debit',
                    numeric: true,
                    ...
                    render: (l: any) => (Number(l.debit) ? money(l.debit, entry.data.currency) : ''),
                  },
                  {
                    key: 'credit',
                    label: 'Credit',
                    ...
                    render: (l: any) => (Number(l.credit) ? money(l.credit, entry.data.currency) : ''),
                  },
                  { key: 'memo', label: 'Memo', value: (l: any) => l.memo ?? '' },
                ]}
              />
              <div className="mt-3 flex gap-6 text-sm">
                <span>
                  <span className="text-ink-subtle">Total debit</span>{' '}
                  <span className="num font-medium">{money(entry.data.totalDebit, entry.data.currency)}</span>
                </span>
                <span>
                  <span className="text-ink-subtle">Total credit</span>{' '}
                  <span className="num font-medium">{money(entry.data.totalCredit, entry.data.currency)}</span>
                </span>
              </div>
            </Card>
          )}
            ...
            <>
              {!trial.data.balanced && (
                ...
                <div className="mb-3 rounded-md border border-danger/40 bg-danger-light p-3 text-sm text-danger">
                  The ledger does not balance. Debits and credits differ by {trial.data.difference}.
                </div>
              )}
              <DataTable
                ...
                columns={[
                  ...
                  { key: 'debit', label: 'Debit', numeric: true, align: 'right', value: (r: any) => Number(r.debit), render: (r: any) => money(r.debit) },
                  { key: 'credit', label: 'Credit', numeric: true, align: 'right', value: (r: any) => Number(r.credit), render: (r: any) => money(r.credit) },
                  {
                    key: 'balance',
                    label: 'Balance',
                    numeric: true,
                    ...
                    render: (r: any) => (
                      <span className={Number(r.balance) < 0 ? 'text-danger' : ''}>
                        {money(r.balance)} <span className="text-[10px] text-ink-subtle">{r.normalSide}</span>
                      </span>
                    ),
                  },
                ]}
              ...
              <div className="mt-3 flex flex-wrap gap-6 text-sm">
                <span>
                  <span className="text-ink-subtle">Total debit</span>{' '}
                  <span className="num font-medium">{money(trial.data.totalDebit)}</span>
                </span>
                <span>
                  <span className="text-ink-subtle">Total credit</span>{' '}
                  <span className="num font-medium">{money(trial.data.totalCredit)}</span>
                </span>
        ...
        <div className="space-y-5">
          ...
          {valuation.data && (
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
              <Stat label="Stock value" value={money(valuation.data.totalValue)} sub={valuation.data.basis} />
              <Stat label="Method" value={valuation.data.method} sub="Costing only — picking stays FEFO" />
          ...
          {reconciliation.data && (
            <Card title="Inventory account vs stock on hand">
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                <Stat label="Ledger balance" value={money(reconciliation.data.ledgerBalance)} />
                <Stat label="Stock valuation" value={money(reconciliation.data.physicalValue)} />
                <Stat
                  label="Difference"
                  value={money(reconciliation.data.difference)}
                  tone={reconciliation.data.withinTolerance ? 'neutral' : 'danger'}
                  sub={
              ...
              <p className="mt-3 text-xs text-ink-muted">{reconciliation.data.note}</p>
            </Card>
          )}
        </div>
      )}
            ...
            (notes.data.length === 0 ? (
              ...
              <DataTable
                ...
                columns={[
                  ...
                  { key: 'noteType', label: 'Type', value: (n: any) => n.noteType, render: (n: any) => <Pill tone={n.noteType === 'CREDIT' ? 'info' : 'warn'}>{n.noteType}</Pill> },
                  { key: 'direction', label: 'Against', value: (n: any) => n.direction },
                  { key: 'noteDate', label: 'Date', value: (n: any) => n.noteDate, render: (n: any) => shortDate(n.noteDate) },
                  { key: 'reason', label: 'Reason', value: (n: any) => n.reason },
                  { key: 'lines', label: 'Lines', numeric: true, align: 'right', optional: true, value: (n: any) => n.lines?.length ?? 0 },
                  {
                    key: 'grandTotal',
                    label: 'Total',
                    numeric: true,
                  ...
                  },
                  {
                    ...
                    render: (n: any) => <Pill tone={STATUS_TONE[n.status] ?? 'neutral'}>{n.status}</Pill>,
                  },
                  {
                    key: 'issue',
                    label: '',
                    render: (n: any) =>
                      n.status === 'DRAFT' ? (
                        <button className="btn-ghost" disabled={busy} onClick={() => void issueNote(n)}>
                          Issue
                        </button>
                      ) : null,
                  },
        ...
        <Card title="Accounting periods">
          ...
          {periods.data &&
            (periods.data.length === 0 ? (
              <Empty>No period has been opened. Without one, entries post without a period check.</Empty>
            ) : (
              <DataTable
                rows={periods.data}
                getKey={(p: any) => p.id}
                ...
                columns={[
                  ...
                  { key: 'startDate', label: 'From', value: (p: any) => p.startDate, render: (p: any) => shortDate(p.startDate) },
                  { key: 'endDate', label: 'To', value: (p: any) => p.endDate, render: (p: any) => shortDate(p.endDate) },
                  {
                    key: 'status',
                    label: 'Status',
                    value: (p: any) => p.status,
                    render: (p: any) => <Pill tone={p.status === 'OPEN' ? 'ok' : 'neutral'}>{p.status}</Pill>,
                  },
                  {
                    key: 'actions',
                    label: '',
                    render: (p: any) =>
                      p.status === 'OPEN' ? (
                        <button className="btn-ghost" disabled={busy} onClick={() => void closePeriod(p)}>
                          Close
                        </button>
                      ) : (
                        <button className="btn-ghost" disabled={busy} onClick={() => void reopenPeriod(p)}>
                          Reopen
                        </button>
                      ),
                  },
