...
export default function RecallsPage() {
  ...
  const detail = useApi<any>(selected ? `/recalls/${selected}` : null, [selected]);
    ...
    <Shell>
      ...
      <div className="grid gap-4 lg:grid-cols-3">
              ...
              {list.data.data.map((r: any) => (
                <button
                  ...
                  className={`w-full rounded-md border p-3 text-left text-sm ${
                    selected === r.id ? 'border-brand bg-brand-light' : 'border-surface-border hover:bg-surface-sunken'
                  }`}
                >
                  <div className="flex items-center justify-between">
                    <span className="font-medium">{r.recallNo}</span>
                    <Pill tone={r.severity === 'CLASS_I' ? 'danger' : 'warn'}>{r.severity}</Pill>
                  </div>
                  <div className="mt-1 text-xs text-ink-muted line-clamp-2">{r.reason}</div>
                  <div className="mt-1 text-xs text-ink-subtle">
                    {shortDate(r.recallDate)} · {r.batches.length} batch(es) ·{' '}
                    {r.tasks.filter((t: any) => t.status === 'PENDING').length} pending
                  </div>
                </button>
              ))}
            </div>
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          {!selected && <Card><Empty>Select a recall to see its dashboard.</Empty></Card>}

          {detail.data && (
            <>
              <Card title={`${detail.data.recall.recallNo} — ${detail.data.recall.severity}`}>
                <p className="text-sm text-ink-muted">{detail.data.recall.reason}</p>
                {detail.data.recall.regulatoryReference && (
                  <p className="mt-1 text-xs text-ink-subtle">
                    Regulatory reference: {detail.data.recall.regulatoryReference}
                  </p>
                )}
                <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
                  {[
                    ...
                    <div key={String(label)} className="rounded-md bg-surface-sunken p-2">
                      <div className="text-xs text-ink-muted">{label}</div>
                      <div className="text-lg font-semibold num">{qty(value)}</div>
                    </div>
                  ))}
                </div>
              </Card>
              ...
              <Card title="Affected batches">
                <Table head={['Batch', 'Product', 'Status', 'In stock', 'Dispensed', 'Recovered']}>
                  {detail.data.batches.map((b: any) => (
                    <tr key={b.batchId}>
                      <td className="td font-medium">{b.batchNumber}</td>
                      <td className="td">{b.product}<div className="text-xs text-ink-subtle">{b.sku}</div></td>
                      <td className="td"><Pill tone="danger">{b.currentStatus}</Pill></td>
              ...
              <Card title={`Recall tasks — ${detail.data.tasks.pending} pending of ${detail.data.tasks.total}`}>
                <TaskList recallId={detail.data.recall.id} onUpdate={updateTask} />
              </Card>
            </>
          )}
        </div>
  ...
  return (
    <div className="space-y-2 text-sm">
      {Object.entries(byType).map(([type, count]) => (
        <div key={type} className="flex items-center justify-between rounded-md bg-surface-sunken px-3 py-2">
          <span className="font-medium">{type.replace(/_/g, ' ')}</span>
          <span className="num text-ink-muted">{String(count)} task(s)</span>
        </div>
      ))}
      <p className="text-xs text-ink-subtle">
        Stock-blocking tasks are created per holding location; patient-notification tasks are
        created from the historical dispensing records for the batch.
      </p>
    </div>
  );
}
