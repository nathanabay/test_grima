...
export default function ReceivingPage() {
  ...
  const recent = useApi<any>(
    warehouseId ? `/goods-receipts?warehouseId=${warehouseId}&pageSize=10` : null,
    [warehouseId, receipt],
  );
  ...
  useEffect(() => {
    ...
    if (first) {
      ...
      setWarehouseId(first.warehouses.find((w: any) => !w.isColdRoom)?.id ?? '');
    }
  }, [org.data]);
  ...
  function applyScan(result: ScanResolution) {
    ...
    setLines((current) => {
      const existing = current.findIndex((l) => l.productId === result.product!.id && !l.batchNumber);
      const filled = {
        productLabel: label,
        batchNumber: result.parsed.batchNumber ?? '',
        expiryDate: result.parsed.expiryDate ? result.parsed.expiryDate.slice(0, 10) : '',
      };
      if (existing >= 0) {
        return current.map((l, i) => (i === existing ? { ...l, ...filled } : l));
      }
      return [
        ...current,
        {
    ...
    if (!result.parsed.isGs1) {
      setError('Not a GS1 DataMatrix: enter and verify the batch and expiry by hand.');
    }
  }
  ...
  const ready =
    lines.length > 0 &&
    lines.every((l) => l.batchNumber && l.expiryDate && Number(l.quantity) > 0 && l.unitCost !== '') &&
    !!po.data?.supplierId;
      ...
      {receipt && (
        <Card className="mb-4" title={`Receipt ${receipt.grnNo} posted`}>
          <p className="text-sm text-ink-muted">
            {receipt.items.length} line(s) received. Batches are QUARANTINED until a QA officer
            releases them.
          </p>
          <div className="mt-2 flex gap-2">
            ...
            <a className="btn-ghost" href="/batches?status=QUARANTINED">Go to quarantine</a>
          </div>
            ...
            <div className="mt-3 rounded-md bg-warn-light p-2 text-xs text-warn">
              ...
              <ul className="mt-1 list-inside list-disc">
                {receipt.items.flatMap((i: any) =>
                  (i.flags ?? []).map((f: string) => <li key={`${i.id}-${f}`}>{i.batchNumber}: {f}</li>),
                )}
              </ul>
            </div>
          )}
        ...
        <Card className="lg:col-span-2" title="Receive a delivery">
          ...
          {lines.length > 0 ? (
            <div className="space-y-3">
              {lines.map((l, i) => (
                <div key={l.key} className="rounded-md border border-surface-border p-3">
                  <div className="mb-2 flex items-center justify-between">
                    <span className="text-sm font-medium">{l.productLabel}</span>
                    <button
                    ...
                    {[
                      ...
                      <div key={field}>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setLines((p) =>
                              p.map((x, xi) => (xi === i ? { ...x, [field as string]: e.target.value } : x)),
                            )
                          }
                        />
                      </div>
                    ...
                    {Number(l.rejectedQty) > 0 && (
                      <div className="sm:col-span-3">
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setLines((p) =>
                              p.map((x, xi) => (xi === i ? { ...x, rejectionReason: e.target.value } : x)),
                            )
                          }
                        />
                      </div>
                  ...
                  {Number(l.rejectedQty) > 0 && (
                    <p className="mt-1 text-xs text-warn">
                      {qty(Number(l.quantity) - Number(l.rejectedQty))} will enter stock;{' '}
                      {qty(Number(l.rejectedQty))} rejected at the door.
                    </p>
                  )}
                </div>
              ))}
              ...
              {!ready && (
                <p className="text-xs text-ink-subtle">
                  Every line needs a purchase order, batch number, expiry, quantity and unit cost.
                </p>
              )}
            </div>
          ) : (
            ...
            <Table head={['GRN', 'When', 'Lines', 'Flags']}>
              {recent.data.data.map((g: any) => (
                <tr key={g.id}>
                  ...
                  <td className="td text-xs text-ink-muted">{shortDate(g.receivedAt)}</td>
