...
const PRODUCT_FIELDS: FieldSpec[] = [
  ...
  { key: 'dosageForm', label: 'Dosage form', required: true, hint: 'Tablet, Capsule, Injection...' },
...
export default function ImportPage() {
  ...
  const validation = useMemo(() => {
    ...
    rows.forEach((raw, index) => {
      ...
      for (const field of PRODUCT_FIELDS) {
        ...
        if (field.required && !value) {
          errors.push({ row: index + 2, field: field.label, message: 'is required but empty' });
          continue;
        }
        if (!value) continue;
        ...
        if (field.numeric) {
          ...
          if (!Number.isFinite(n)) {
            errors.push({ row: index + 2, field: field.label, message: `"${value}" is not a number` });
            continue;
          }
          if (n < 0) {
            errors.push({ row: index + 2, field: field.label, message: 'cannot be negative' });
            continue;
          }
          record[field.key] = n;
        } else {
    ...
    mapped.forEach((r, i) => {
      if (!r.sku) return;
      if (seen.has(r.sku)) {
        errors.push({ row: i + 2, field: 'SKU', message: `duplicates row ${seen.get(r.sku)}` });
  ...
  function downloadTemplate() {
    const csv = [
      ...
      PRODUCT_FIELDS.map((f) =>
        f.key === 'sku' ? 'SKU-0001'
        : f.key === 'genericName' ? 'Amoxicillin'
        : f.key === 'brandName' ? 'Amoxil'
        : f.key === 'activeIngredient' ? 'Amoxicillin trihydrate'
        : f.key === 'strength' ? '500 mg'
        : f.key === 'dosageForm' ? 'Capsule'
        : f.key === 'baseUnit' ? 'CAPSULE'
        : f.key === 'taxRate' ? '0.15'
        : f.numeric ? '0' : '',
      ).join(','),
    ].join('\r\n');
  ...
  async function runImport() {
    ...
    setError(null);
    try {
      setResult(await api('/products/import', { method: 'POST', body: { rows: validation.mapped } }));
      setStep('done');
    } catch (e: any) {
      ...
      setError(
        serverErrors
          ? `${e.message}\n${serverErrors.slice(0, 10).map((x: any) => `Row ${x.row}: ${x.error}`).join('\n')}`
          : e.message,
      );
    } finally {
      setBusy(false);
  ...
  return (
    <Shell>
      <PageHeader
        ...
        action={<button className="btn-ghost" onClick={downloadTemplate}>Download template</button>}
      />

      <ol className="mb-4 flex flex-wrap gap-1 text-xs">
        {(['upload', 'map', 'preview', 'done'] as Step[]).map((s) => (
          <li key={s} className={`rounded px-2 py-1 ${step === s ? 'bg-brand text-white' : 'bg-surface-sunken text-ink-subtle'}`}>
            {s.toUpperCase()}
          </li>
        ))}
      </ol>
      ...
      {step === 'upload' && (
        <Card title="Upload a CSV">
          <input type="file" accept=".csv,text/csv" onChange={(e) => { const f = e.target.files?.[0]; if (f) loadFile(f); }} />
          <p className="mt-2 text-xs text-ink-subtle">
            The first row must be a header. Download the template above for the expected columns.
          </p>
        </Card>
      )}
        ...
        <Card title={`Map columns — ${fileName} (${rows.length} rows)`}>
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {PRODUCT_FIELDS.map((f) => (
              <div key={f.key}>
                ...
                {f.hint && <p className="mt-0.5 text-xs text-ink-subtle">{f.hint}</p>}
              </div>
            ))}
          </div>
          ...
          {missingRequired.length > 0 && (
            <p className="mt-3 text-sm text-danger">
              Map these required fields first: {missingRequired.map((f) => f.label).join(', ')}
            </p>
          )}

          <button className="btn-primary mt-3" disabled={missingRequired.length > 0} onClick={() => setStep('preview')}>
      ...
      {step === 'preview' && (
        <div className="space-y-4">
          <Card title={`Validation — ${validation.errors.length} problem(s) in ${rows.length} rows`}>
            {validation.errors.length ? (
              <>
                ...
                <div className="mt-3 flex gap-2">
                  <button className="btn-ghost" onClick={downloadErrors}>Download error report</button>
                  <button className="btn-ghost" onClick={() => setStep('map')}>Back to mapping</button>
                </div>
                <p className="mt-2 text-xs text-danger">
                  Nothing will be imported while any row is invalid — a partly imported drug master
                  is worse than none.
                </p>
              </>
            ) : (
          ...
          <Card title="Preview (first 10 rows)">
            <Table head={PRODUCT_FIELDS.filter((f) => mapping[f.key]).map((f) => f.label)}>
              {validation.mapped.slice(0, 10).map((r, i) => (
                <tr key={i}>
                  {PRODUCT_FIELDS.filter((f) => mapping[f.key]).map((f) => (
                    <td key={f.key} className={`td ${f.numeric ? 'num' : ''}`}>{String(r[f.key] ?? '')}</td>
                  ))}
                </tr>
              ))}
            </Table>
            <button className="btn-primary mt-3" disabled={busy || validation.errors.length > 0} onClick={runImport}>
              {busy ? 'Importing...' : `Import ${validation.mapped.length} product(s)`}
            </button>
          </Card>
        </div>
      )}
      ...
      {step === 'done' && result && (
        <Card title="Import complete">
          ...
          <button className="btn-ghost mt-3" onClick={() => { setStep('upload'); setRows([]); setHeaders([]); setResult(null); }}>
            Import another file
          </button>
        </Card>
      )}
